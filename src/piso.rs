...
trait ShiftRegisterInternal {
    fn update(&self, index: usize, command: bool) -> Result<bool, ()>;
}
...
impl InputPin for ShiftRegisterPin<'_> {
    type Error = ();
...
macro_rules! ShiftRegisterBuilder {
    ($name: ident, $size: expr) => {
        ...
        where
            ...
            Pin3: InputPin,
        {
            /// Reads the value of the shift register output at `index` to value `command`
            fn update(&self, index: usize, command: bool) -> Result<bool, ()> {
                self.latch.borrow_mut().set_low().map_err(|_e| ())?;
                let len = self.input_state.borrow().len();

                for i in 0..len {
                    self.input_state.borrow_mut()[i] =
                        self.data.borrow_mut().is_high().map_err(|_e| ())?;
                ...
                let mut result = self.input_state.borrow_mut()[index];
                if !command {
                    result = !result;
                }
                Ok(result)
            ...
            Pin3: InputPin,
        {
            ...
            pub fn new(clock: Pin1, latch: Pin2, data: Pin3) -> Self {
                $name {
                    ...
                    input_state: RefCell::new([false; $size]),
                }
            }
...
/// 8 output parallel-in serial-out shift register
